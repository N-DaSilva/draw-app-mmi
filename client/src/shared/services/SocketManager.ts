...
import type { Drawpoint, Point, DrawPoint } from '../types/drawing.type';
...
export type SocketServerToClientEvents = {
  ...
  'draw:start': (payload: DrawPoint) => void; 
  'draw:move': (payload: DrawPoint) => void; 
  'draw:end':(payload: DrawPoint) => void; 
}

export type GetEndpoints = {
  'users': {users: User[]};
  'strokes': {strokes: DrawPoint[]}
}
